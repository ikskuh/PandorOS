...
extern optiongroup_t halOptions;
...
struct ModeInfoBlock {
  ...
  uint32_t realFctPtr; // VBE_FAR(realFctPtr);
...
void x86video_init(struct multiboot_info const * info)
{
	...
	if((info->flags & MULTIBOOT_INFO_VIDEO_INFO) == 0) {
		return;
	}
	
	hal_debug("Got a video mode kernel!\n");
	
	struct ModeInfoBlock const * mib = (void const *)info->vbe_mode_info;
...
static unsigned char g_8x8_font[];
static unsigned char g_8x16_font[];
